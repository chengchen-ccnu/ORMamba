...
class MTLPair_Dataset(torch.utils.data.Dataset):
    ...
    def extract_samples(self, base_dir="./data/MTL-AQA/new_total_frames_256s"):
        """
        根据 label_dict 中的起止帧号，将样本帧复制到新的子文件夹。
        """
        for (match_id, sample_id), frame_info in self.label_dict.items():
            start_frame = frame_info['start_frame']
            end_frame = frame_info['end_frame']

            # 比赛编号补0为两位，如 1 -> "01"
            match_dir = f"{match_id:02d}"
            src_dir = os.path.join(base_dir, match_dir)

            # 目标路径：./data/.../{比赛编号两位}/{样本编号}
            dst_dir = os.path.join(src_dir, str(sample_id))
            os.makedirs(dst_dir, exist_ok=True)

            print(f"📁 处理样本 ({match_id}, {sample_id})：复制 {start_frame}~{end_frame}")
            ...
            for frame_id in range(start_frame, end_frame + 1):
                ...
                if os.path.exists(src_path):
                    ...
                    print(f"⚠️ 缺失帧: {src_path}")

        print("✅ 全部样本处理完成！")

    def load_video(self,video_file_name, phase):
        image_list = sorted((glob.glob(os.path.join(self.data_root, f"{video_file_name[0]:02d}", str(video_file_name[1]), '*.jpg'))))

        start_frame = int(image_list[0].split("\\")[-1][:-4]) + 1   #   去掉第一帧
    ...
    def process_robust(self):
        ...
        for difficulty, file_list in self.difficulties_dict.items():
            # 提取该难度下所有分数
            scores = [self.label_dict[fname]['final_score'] for fname in file_list]
